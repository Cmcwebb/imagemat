...
$saveto_choice= getparameter('saveto');
//var_dump($_POST);

if ((isset($annotation_id)) && (isset($saveto_choice))){
  header('Window-Target: _top');
   header('Location: ' . dirname($_SERVER['REQUEST_URI']) . '/../project/save_to_project.php?annotation_id='.$annotation_id.'');
      return;
}
...
';
?>
function load_rightsubframe()
{
  var load = document.getElementById("load_right");
  if (!load) {
...
<body onload="load_rightsubframe()">
<?php
bodyHeader();
// print_r($_POST);
...
<div id="splitpane">
<iframe id="imageFrame" name="imageFrame" class="imageFrame" style="background-color: #fff" mozallowfullscreen webkitallowfullscreen></iframe>
<iframe id="annotateFrame" name="annotateFrame" class="annotateFrame" mozallowfullscreen webkitallowfullscreen></iframe>
<iframe id="extraframe" name="extraframe" class="imageFrame" style="display:none" mozallowfullscreen webkitallowfullscreen></iframe>
</div>

<?php
echo '
<form id="load_right" name="load_right" method="post" target="annotateFrame" action="';
if (!isset($annotation_id)) {
