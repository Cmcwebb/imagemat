...
var isOpera = !!window.opera,
	...
	curConfig = { /* ijd */
		...
		dimensions: [top.markup_background.image.naturalWidth,
					 top.markup_background.image.naturalHeight]
	};
...
// Create Root SVG element. This is a container for the document being edited,
...
var image   = top.markup_background.image;
...
this.setMarkupData = function(options)
{
    var image =  top.markup_background.image;
	var title, description;

	if (image) {
		node   = document.getElementById('canvas_title');
		title  = node.value;
		image.title = trim(title);

		var editor = CKEDITOR.instances['canvas_desc'];
	   	description = editor.getData();
		image.description = trim(description);

		if(options) $.extend(save_options, options);
		...
		var layers  = all_layers.length;
		var at, max, i, layer, markups, markup, svg, child;

		if (typeof image.markups == 'undefined') {
			markups = null;
			max     = 0;
		} else {
		...
		for (i = at = 0; i < layers; ++i) {
			layer       = all_layers[i];
			title       = trim(layer.title);
			description = trim(layer.description);
			svg         = '';

			for (child = layer.svg.firstChild; child; child = child.nextSibling) {
				if (child.nodeType == 1) { // element node
					if (child.tagName == 'title') {
						continue;
				}	}
				svg    += svgToString(child,0);
			}
			if (svg == '') {
				if (description == '') {
					continue;
...
var addSvgElementFromJson = this.addSvgElementFromJson = function(data) {
	...
	if (shape && data.element != shape.tagName) {
		current_svg.removeChild(shape);
		shape = null;
	}
	if (!shape) {
		shape = svgdoc.createElementNS(svgns, data.element);
		if (current_svg) {
			current_svg.appendChild(shape);
		}
	}
	if(data.curStyles) {
...
// intersect the multi-select rubber-band-box on the current_svg only.
// 
...
var getIntersectionList = this.getIntersectionList = function(rect) {
	...
	if(!curBBoxes.length) {
		// Cache all bboxes
		curBBoxes = getVisibleElements(current_svg, true);
	}
	
	var resultList = null;
	try {
		resultList = current_svg.getIntersectionList(rect, null);
	} catch(e) { }
...
this.selectAllInCurrentLayer = function() {
	if (current_svg) {
		clearSelection();
		addToSelection($(current_svg).children());
		current_mode = "select";
		call("selected", selectedElements);			
	}
};
...
this.getSourceView = function()
{
	...
	var layers = all_layers.length;
	var ret    = '';
	var i, svg, ok, g, layer;

	ret += '<span><font color="red"><h3 align="left">All SVG</h3></font>';
	svg = svgToString(svgcontent, 0);
	ret += '<p align="left"><svg class="svg_source_icon" xmlns="http://www.w3.org/2000/svg" version="1.1" x="0" y="0" width="5%" height="5%" viewBox="0 0 ' + width + ' ' + height + '">' + svg + '</svg>' + escapeHTML(svg) + '</p></span><br class="clear" />';

	ok = true;
	g  = svgcontent.firstChild;
	for (i = 0; i < layers; ++i) {
		layer = all_layers[i];
		ret += '<span>';
		if (i == current_index) {
		...
		ret += '<h3 align="left">' + escapeHTML(layer.title) + '</h3>';
		if (i == current_index) {
			ret += '</font>';
		}
		...
		svg = layer.svg;
		if (ok && g != svg) {
			alert('svgcontent out of sync');
			ok = false;
		}
 
		svg = svgToString(svg, 0);
		ret += '<p align="left"><svg class="svg_source_icon" xmlns="http://www.w3.org/2000/svg" version="1.1" x="0" y="0" width="5%" height="5%" viewBox="0 0 ' + width + ' ' + height + '">' + svg + '</svg>' + escapeHTML(svg) + '</p></span><br class="clear" />';
		if (ok) {
			g = g.nextSibling;
	}	}
	if (ok) {
		if (g != null) {
			alert('svgcontent has additional nodes');
			ok = false;
	}	}
	ret += '<br/>';
	return ret;
...
var removeChildren = function(parent)
{
	...
	while ((node = parent.firstChild)) {
		parent.removeChild(node);
	}
};

this.noLayers = function()
{
	var layerlistbody = document.getElementById('layerlistbody');
	var selLayerNames = document.getElementById('selLayerNames');
	var svgcontent    = document.getElementById('svgcontent');

	removeChildren(layerlistbody);
	removeChildren(selLayerNames);
	removeChildren(svgcontent);
...
var saveLayerText = function()
{
	if (current_index < 0) {
		return;
	}
	var editor = CKEDITOR.instances['layersText'];

	if (!editor) {
		//alert('No editor in saveLayerText');
		return;
	}
	if (!editor.checkDirty()) {
		//alert('Editor not dirty');
		return;
	}
	all_layers[current_index].description = editor.getData();
};

var setLayerText = function()
{
	if (current_index < 0) {
		return;
	}
	...
	var text = all_layers[current_index].description;
	if (!text) {
		text = '';
	}
	editor.setData(text);
...
var findLayer = function(td)
{
	var tr  = td.parentNode;
	var lth = all_layers.length;
	var i;

	for (i = 0; i < lth; ++i) {
		if (all_layers[i].tr == tr) {
			return i;
	}	}
	return -1;
};

var setLayerVisibility = function(index, bVisible)
{
	var svg = all_layers[index].svg;
	if (svg) {
		var oldDisplay = svg.getAttribute("display");
		if (!oldDisplay) {
			oldDisplay = "inline";
		}
		svg.setAttribute("display", bVisible ? "inline" : "none");
		addCommandToHistory(new ChangeElementCommand(svg, {"display":oldDisplay}, "Layer Visibility"));
...
var clickLayerVisible = function(evt)
{
	var index = findLayer(this);
	if (0 <= index) {
		var svg = all_layers[index].svg;
		var vis = (svg.getAttribute("display") == 'none');
		setLayerVisibility(index, vis);
		var display = (vis ? 'inline' : 'none');
		var child;
...
this.setNewCurrentLayer = function(index)
{
	if (index != current_index) {
		if (0 <= current_index) {
			all_layers[current_index].tr.className = 'layer';
			saveLayerText();
			if (selectedElements[0] != null) {
				clearSelection();
			}
			current_svg.setAttribute("style", "pointer-events:none");
		}
		current_index = index;
		if (index < 0) {
			current_svg = null;
		} else {
			all_layers[index].tr.className = 'layer layersel';
			current_svg   = all_layers[index].svg;
			current_svg.setAttribute("style", "pointer-events:all");
			setLayerText();
	}	}
};
...
var removeLayer = function(index)
{
	...
	var svg       = layer.svg;
	var new_layer = index + 1;
	if (all_layers.length <= new_layer) {
		new_layer = index - 1;
	}

	canvas.setNewCurrentLayer(new_layer);
	svgcontent.removeChild(svg);
	all_layers.splice(index, 1);
	if (index < current_index) {
		--current_index;
	}
	var parent = document.getElementById('layerlistbody');
	parent.removeChild(layer.tr);
	parent = document.getElementById('selLayerNames');
	parent.removeChild(layer.option);
	call("changed", [ svgcontent ]);
};

var clickLayerName = function(evt)
{
	var index = findLayer(this);
	if (0 <= index) {
		canvas.setNewCurrentLayer(index);
	}
	evt.preventDefault();
};
...
var toggleHighlightLayer = function(index)
{
	var lth = all_layers.length;
	var opacity;
	for (var i = 0; i < lth; ++i) {
		...
		all_layers[i].svg.setAttribute("opacity", opacity);
	}
};
		
var mouseoverLayerName = function(evt)
{
	var index = findLayer(this);

	$(this).css({"font-style": "italic", "color":"blue"});
	toggleHighlightLayer(index);
};

var mouseoutLayerName = function(evt)
{
	$(this).css({"font-style": "normal", "color":"black"});
	toggleHighlightLayer();
};

var insertLayerBefore = function(index, layer)
{
	var layerListBody  = document.getElementById('layerlistbody');
	var selLayerNames  = document.getElementById('selLayerNames');
	var isAtEnd        = (index == all_layers.length);

	if (isAtEnd) {
		layerListBody.appendChild(layer.tr);
		selLayerNames.appendChild(layer.option);
		svgcontent.appendChild(layer.svg);
		...
		before = null;
		i      = 0;
		for (node = layerListBody.firstChild; node; node = node.nextSibling) {
			if (node.tagName == 'TR') {
				if (i == index) {
					before = node;
					break;
		...
		layerListBody.insertBefore(layer.tr, before);

		before = null;
		i      = 0;
		for (node = selLayerNames.firstChild; node; node = node.nextSibling) {
		...
		selLayerNames.insertBefore(layer.option, before);

		before = null;
		i      = 0;
	    for (node = svgcontent.firstChild; node; node = node.nextSibling) {
			if (node.tagName == 'g') {
				if (i == index) {
					before = node;
					break;
		...
		svgcontent.insertBefore(layer.svg, before);

		all_layers.splice(index, 0, layer);
		if (index <= current_index) {
			++current_index;
		}
	}
...
this.appendLayer = function(title, g, description)
{
	walkTree(g, function(e){
	...
	img.src = '../images/annotate/eye.png';
	img.style.width   = 17;
	img.style.height  = 17;
	img.style.display = '';
	td.appendChild(img);
	img = document.createElement('img');
	img.src = '../images/annotate/eye-close.png';
	img.style.width   = 17;
	img.style.height  = 17;
	...
	var layer = {tr:tr, option:option, title:title, svg:g, description:description };
	insertLayerBefore(lth, layer);
}

this.defaultLayer = function()
{
    canvas.noLayers();
    canvas.appendLayer('Object 1', svgdoc.createElementNS(svgns, "g"));
};

var CreateLayerCommand = this.undoCmd.createLayer = function(title)
{
	this.title = title;
	this.svg   = svgdoc.createElementNS(svgns, "g");
	this.text  = "Create layer ";
	
	this.apply = function()
	{ 
		clearSelection();
		canvas.appendLayer(this.title, this.svg, null);
	};

	this.unapply = function()
	{
		removeLayer(all_layers.length - 1);
	};
...
var InsertLayerBeforeCommand = this.undoCmd.insertLayer = function(index, layer)
{
	this.index = index;
	this.layer = layer;
	this.text  = "Insert layer " + index;
	
	this.apply = function()
	{ 
		insertLayerBefore(this.index, this.layer);
	};

	this.unapply = function()
	{
		removeLayer(this.index);
	};
...
var DeleteLayerCommand = this.undoCmd.deleteLayer = function(index)
{
	this.index  = index;
	this.layer  = all_layers[index];
	this.text   = 'Delete Layer ' + index;
...
var RenameLayerCommand = this.undoCmd.renameLayer = function(newName)
{
	this.index  = current_index;
	this.oldName = all_layers[current_index].title;
	this.newName = newName;
	this.text   = "Rename layer " + newName;
	
	var renameTo = function(index, newName)
	{
		var layer  = all_layers[index];
		var tr     = layer.tr;
	...
	this.apply = function()
	{ 
		renameTo(this.index, this.newName);
	};

	this.unapply = function()
	{
		renameTo(this.index, this.oldName);
	};
...
{
	var create = new CreateLayerCommand(title);
	create.apply();
	addCommandToHistory(create);
};

this.deleteCurrentLayer = function()
{
	if (0 <= current_index && all_layers.length > 1) {
		var deleteLayerCommand = new DeleteLayerCommand(current_index);
		deleteLayerCommand.apply();
		addCommandToHistory(deleteLayerCommand);
		return true;
	}
	return false;
};
...
this.enableCurrentSelectOption = function()
{
	if (0 <= current_index) {
		$('#selLayerNames').removeAttr('disabled').val(current_index);
	}
};

this.disableSelectOptions = function()
{
	$('#selLayerNames').attr('disabled', 'disabled');
};

this.getLayerTitle = function(i)
{
	if (i >= 0 && i < canvas.getNumLayers()) {
		return all_layers[i].title;
	}
	return null;
};

this.getCurrentLayerTitle = function()
{
	return canvas.getLayerTitle(current_index);
}
...
this.renameCurrentLayer = function(newname)
{
	...
	var renameLayerCommand = new RenameLayerCommand(newname);

	renameLayerCommand.apply();
	addCommandToHistory(renameLayerCommand);
};

this.setCurrentLayerPosition = function(shift)
{
	if (current_index < 0) {
		return false;
	}
	var newpos         = current_index + shift;
	var batchCommand, layer;

	switch (shift) {
	case 1:
		if (all_layers.length <= newpos) {
			return false;
		}
		batchCommand = new BatchCommand('Move layer down');
		break;
	case -1:
		if (newpos < 0) {
			return false;
		}
		batchCommand = new BatchCommand('Move layer up');
		break;
	default:
		return false;
	}

	layer = all_layers[current_index];
	batchCommand.addSubCommand( new DeleteLayerCommand(current_index));
	batchCommand.addSubCommand( new InsertLayerBeforeCommand(newpos, layer));
	batchCommand.apply();
	addCommandToHistory(batchCommand);
	return true;
};
...
this.moveSelectedToLayer = function(index)
{
	if (index < 0 || all_layers.length <= index) {
		return false;
	}
	var layer = all_layers[index].svg;
...
this.getVersion = function() {
	return "svgcanvas.js ($Rev: 1738 Hacked for ImageMAT $)";
};
...
this.getDocumentTitle = function() {
	for (var child = svgcontent.firstChild; child; child = child.nextSibling) {
...
// Function: getDocumentDesc
// Returns the current document description or an empty string if not found
this.getDocumentDesc = function() {
	for (var child = svgcontent.firstChild; child; child = child.nextSibling) {
...
// Function: setDocumentDesc
// Adds/updates a desc element for the document with the given name.
// This is an undoable action
//
// Parameters:
// newdesc - String with the new desc
this.setDocumentDesc = function(newdesc) {
	var doc_desc = false, old_desc = '';
	var child;
	
	var batchCmd = new BatchCommand("Change Image Desc");
	
	for (child = svgcontent.firstChild; child; child = child.nextSibling) {
		if(child.nodeName == 'desc') {
			doc_desc = child;
			old_desc = doc_desc.textContent;
			break;
		}
	}
	if(!doc_desc) {
		doc_desc = svgdoc.createElementNS(svgns, "desc");
		child    = svgcontent.firstChild;
		if (child && child.nodeName == 'title') {
			child = child.nextSibling;
		}
		svgcontent.insertBefore(doc_desc, child);
	} 
	
	if(newdesc.length) {
		doc_desc.textContent = newdesc;
	} else {
		// No desc given, so element is not necessary
		doc_desc.parentNode.removeChild(doc_desc);
	}
	batchCmd.addSubCommand(new ChangeElementCommand(doc_desc, {'#text': old_desc}));
	addCommandToHistory(batchCmd);
}
...
this.setBBoxZoom = function(val, editor_w, editor_h) {
	...
	switch (val) {
		...
		case 'clip':	/* ijd */
			...
			var scale  = editor_w  / naturalWidth;
			var scale1 = editor_h  / naturalHeight;
			if (scale1 < scale) {
				// Fraction of height showing 
				bb = {width:naturalWidth,
					  height:(editor_h / ( scale * naturalHeight)),
					  x:0,
					  y:0
					 }
			} else {
				// Fraction of width showing
				bb = {width:(editor_w / ( scale1 * naturalWidth)),
		...
		case 'content':
			...
			break;
		case 'layer':
			bb = getStrokedBBox(getVisibleElements(current_svg));
			break;
		default:
			return;
	}
...
this.cycleElement = function(next) {
	...
	var elem = false;
	var all_elems = getVisibleElements(current_svg);
	if (cur_elem == null) {
