...
var forceSaveMarkup = null;
var cancelSaveMarkup = null;
...
(function() { 
	...
	if(!window.svgEditor) window.svgEditor = function($) {
		var svgCanvas;
		var Editor = {};
		var markup_background = top.markup_background;
		var image = markup_background.image;
		var is_ready = false;
		
		var defaultPrefs = {
			lang:markup_background.lang,	/* ijd */
			iconsize:'m',
			bkgd_color:'#FFF',
			bkgd_url:'',
			img_save:'embed'
			...
			curConfig = {
				canvas_expansion: 1, /* ijd */
				dimensions: [image.naturalWidth,
							 image.naturalHeight], /* ijd */
				initFill: {
			...
			"layer":"Object",
			"enterUniqueLayerName":"Please enter a unique object name",
			"enterNewLayerName":"Please enter the new object name",
			"layerHasThatName":"Object already has that name",
			"QmoveElemsToLayer":"Move selected elements to object \"%s\"?",
		...
		Editor.init = function() {
			...
			$.svgIcons(curConfig.imgPath + 'svg_edit_icons.svg', {
				...
				placement: {
					...
					'#tool_save div':'save',
					'#tool_msave div':'msave',
					'#tool_return div':'return',
					'#tool_quit div':'quit',
					'#tool_source':'source',
					...
					'#tool_docprops_save':'ok',
					'#tool_source_cancel':'close',
					'#tool_docprops_cancel':'cancel',
...
// 					}
				...
				if ( (elem && !is_node)	|| multiselected) {
					svgCanvas.enableCurrentSelectOption();
				} else {
					svgCanvas.disableSelectOptions();
				}
			};
			...
			$('#selLayerNames').change(function()
			{
				...
				var title = svgCanvas.getLayerTitle(index);
				var confirm_str = uiStrings.QmoveElemsToLayer.replace('%s', title);
				var moveToLayer = function(ok) {
					if(!ok) return;
					promptMoveLayerOnce = true;
					svgCanvas.moveSelectedToLayer(index);
			...
			*/
			...
			var getTab = function() {
				return  markup_background.image.tab;
			}
			
			var clickQuit = cancelSaveMarkup = function(){
			    var location = '../annotate/images.php?loaded=Y';
				var tab   = getTab();
				if (tab) {
					location += '&tab=' + tab;
				}
			...
			var saveMarkup = function()
			{
				...
				var image = svgCanvas.setMarkupData(saveOpts);
				show_save_warning = false;
				return image;
			}

			var clickReturn = forceSaveMarkup = function(){
				saveMarkup();
				clickQuit();
			};

			var clickMinorSave = function(){
				var image    = saveMarkup();
				var tab      = getTab();
				var post     = 
{ action:'../annotate/images.php',
  parameters:{loaded:'Y', tab:tab }
};
				if (image.markup_id) {
					var image1  = sendMarkup(image);
					if (image1) {
						post.parameters.minor='Y';
						post.parameters.markup = JSON.stringify(image1);
				}	}
				submitPost(post);
			};
  ...
  parameters:{loaded:'Y', tab:tab}
				...
				if (image.markup_id) {
					...
					if (image1) {
						post.parameters.markup = JSON.stringify(image1);
						if (image.maxversion) {
							image.version = ++image.maxversion;
				}	}	}
				submitPost(post);
			};
		...
		// 		updateBgImage();
			...
			var showSourceEditor = function(){
				...
				editingsource = true;
				var str = svgCanvas.getSourceView();
				var div = document.getElementById('svg_source_textarea');
				div.innerHTML = str;
			...
			var showDocProperties = function(){
				...
				var text = svgCanvas.getDocumentTitle();
				$('#canvas_title').val(text);
				text = svgCanvas.getDocumentDesc();
				CKEDITOR.instances['canvas_desc'].setData(text);
			...
			var setDesc = function(desc) {
				var editor_desc = $('desc:first').text().split(':')[0];
				var new_desc = editor_desc + (desc?': ' + desc:'');
				$('desc:first').text(new_desc);
			}
			
			var saveDocProperties = function(){
				// set desc (Do first so ends up second in list
				var new_desc = CKEDITOR.instances['canvas_desc'].getData();
				setDesc(new_desc);
				svgCanvas.setDocumentDesc(new_desc);
...
*/
			...
			var setIcon = Editor.setIcon = function(elem, icon_id, forcedSize) {
				...
				if (icon == null) {
					alert('Editor.setIcon: icon null');
				}
				icon = icon.clone();
			...
			var setIconSize = Editor.setIconSize = function(size, force) {
				...
				// Change icon size
				$('.tool_button, .push_button, .tool_button_current, .disabled, .icon_label, #url_notice')
...
*/
			...
			(function() {
				var toolnames = ['clear','open','save','quit','source','delete','delete_multi','paste','clone','clone_multi','move_top','move_bottom'];
				var all_tools = '';
			...
			// in Opera and Chrome
			if (isMac) {
				var shortcutButtons = ["tool_clear", "tool_save", "tool_msave", "tool_return", "tool_docprops", "tool_quit", "tool_source", "tool_undo", "tool_redo", "tool_clone"];
...
><rect width="100%" height="100%" fill="#' + curConfig.initFill.color + '" opacity="' + curConfig.initFill.opacity + '"\
/><linearGradient id="gradbox_"\
><stop stop-color="#000" offset="0.0"\
/><stop stop-color="#FF0000" offset="1.0"\
/> </linearGradient></svg>';
			var svgdocbox = text2xml(svgxml);
			...
			$('#layer_new').click(function() {
				var layers   = svgCanvas.getNumLayers();
				var curNames = new Array(layers);
				for (var i = 0; i < layers; ++i) {
					curNames[i] = svgCanvas.getLayerTitle(i);
				}
			...
			$('#layer_rename').click(function() {
				var oldName = svgCanvas.getCurrentLayerTitle();
				if (oldName == null) {
					// There are no layers to rename
					return;
				}
			...
			var SIDEPANEL_OPENWIDTH = 300; /* IJD */
			...
			var resizePanel = function(evt) {
				...
				var oldLeft    = parseInt(workarea.css('width'));
				var oldWidth   = parseInt(sidepanels.css('width'));
				...
				if (newWidth == 2) {
					...
					sidepanels.css('width', 2);
					workarea.css('right', 2);
				} else {
					workarea.css('right',   newWidth);
					sidepanels.css('width', newWidth);
					layerpanel.css('width', newWidth-2);
					layerpanel.css('display', '');
				}
				sidedrag += oldWidth - newWidth;
			}
			...
			var toggleSidePanel = function(close)
			{
				...
				if (close || layerpanel.css('display') != 'none') {
					layerpanel.css('display', 'none');
					sidepanels.css('width', 2);
					workarea.css('right', 2);
				} else {
					workarea.css('right',   SIDEPANEL_OPENWIDTH);
					sidepanels.css('width', SIDEPANEL_OPENWIDTH);
					layerpanel.css('width', SIDEPANEL_OPENWIDTH-2);
					layerpanel.css('display', '');
				}
			};
			
			var getMarkupData = function()
			{
  				label_frame_button();

  				var markup_background = top.markup_background;
  				var lang   = markup_background.lang;

  				createEditor('layersText',  false, lang, false);
  				createEditor('canvas_desc', false, lang, false);

			    var image  = markup_background.image;
				var layers = -1;
			
				if (!svgCanvas) {
					alert('getMarkupData: svgCanvas not defined');
					return;
				} 
				svgCanvas.noLayers();
				if (image) {
  					// Svg produces floating point inaccuracies
					// in width and height. Can't get accurate size
					// from background image
  					var canvas_size = document.getElementById('canvas_size');
  					if (canvas_size) {
						canvas_size.textContent = 'Size: ' + image.naturalWidth + 'x' + image.naturalHeight;
  					}
					if (image.markup_id) {
						// Can't save markup if annotationofurl not yet saved
						var button = document.getElementById('tool_save');
						button.style.display = '';
						button = document.getElementById('tool_msave');
						button.style.display = '';
  					}
					...
					if (description && description != '') {
						node = document.getElementById('canvas_desc');
						node.value = description;
					}

					var markups = image.markups;
					var layer, markup, svg1;

					if (markups) {
						layers = markups.length;
					}
					for (layer = 0; layer < layers; ++layer) {
						title = markups[layer].title;
						if (!title) {
							title = 'Object ' + layer;
						}
						svg1 = markups[layer].svg;
						if (!svg1) {
							svg1 = '';
						}
						svg += '<g>' + svg1 + '</g>';
...
'<svg xmlns="http://www.w3.org/2000/svg"\
      ...
      xmlns:math="http://www.w3.org/1998/Math/MathML" >' + svg + '</svg>';
						...
						while ((child = svgcontent.firstChild)) {
							svgcontent.removeChild(child);
						}
						for (layer = 0; (child = node.firstChild); ) {
							node.removeChild(child);
							if (child.tagName != 'g') {
								alert('child.tagName = ' + child.tagName);
								continue;
							}
							svgcontent.appendChild(child);
							svgCanvas.appendLayer(markups[layer].title, child, markups[layer].description);
							++layer;
						}
						if (layer != layers) {
							alert('Failed to parse all svg layers');
						}
					}
				}
				if (layers <= 0) {
				...
				svgCanvas.setNewCurrentLayer(0);
			}
			...
			var Actions = function() {
				...
				var tool_buttons = [
					...
					{sel:'#tool_save', fn: function() { clickSave()}, evt: 'mouseup', key: [modKey+'S', true]},
					{sel:'#tool_msave', fn: function() { clickMinorSave()}, evt: 'mouseup', key: [modKey+'M', true]},
					{sel:'#tool_return', fn: function() { clickReturn()}, evt: 'mouseup', key: [modKey+'R', true]},
					{sel:'#tool_quit', fn: function() { clickQuit()}, evt: 'mouseup', key: [modKey+'Q', true]},
							...
							if(opts.key) {
								...
								if(opts.sel && !opts.hidekey) {
									...
									if(!btn.parents('#main_menu').length) {
										...
										if (new_title) {
											new_title = new_title.split('[')[0] + '[' + keyval + ']';
		...
		//	$('#copyright')[0].setAttribute("title", revnums);
			...
			var good_langs = [];
			var lang       = top.markup_background.lang;
			...
			Editor.setLang = function(lang, strings) {
				...
				if(strings) {
					...
					var oldLayerName = svgCanvas.getCurrentLayerTitle();
